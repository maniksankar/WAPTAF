...
Test Case Client Os
    [Tags]    clientos   CLIENT_TEST
    ${ret}    Get Client Os    ap_lan_client_1
    Log To Console    AP_LAN_CLIENT_1 : ${ret}

Test Case Get Client Ipv4 Address
    [Tags]    Getipv4   CLIENT_TEST
    ${ret}    Get Client Ipv4    ap_lan_client_1
    Log To Console    AP_LAN_CLIENT_1 IPV4 address : ${ret}

Test Case Client Radio State
    [Tags]   Radiostate   CLIENT_TEST
...
Test Case Client Interface State
    [Tags]   Interfacestate   CLIENT_TEST
    Set Client Radio State   ap_wlan_client_1  ${1}
    Sleep  3
...
Test Case Get Client Ssid Availability
    [Tags]  ssidavailability    CLIENT_TEST
    Set Client Radio State   ap_wlan_client_1  ${1}
    Sleep  3
    ${ret}  Get Client Radio State   ap_wlan_client_1
    Should Be Equal As Integers    ${1}    ${ret}
    Check Ap Ssid Visibility  ap_wlan_client_1  ${SSID}
    ${ret}  Get Ap Bssid Visibility  ap_wlan_client_1  ${SSID}
    Should Not Be Empty  ${ret}
    Log To Console  \nBSSID:${ret}

Test Case Connect Client To Ssid
    [Tags]  connectssid   CLIENT_TEST
    Set Client Radio State   ap_wlan_client_1  ${1}
    Sleep  3
    ${ret}  Get Client Radio State   ap_wlan_client_1
    Should Be Equal As Integers    ${1}    ${ret}
