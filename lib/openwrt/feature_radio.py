...
class FeatureRadio(BaseFeatureRadio,
    ...
    def __init__(self):
        zi_logger.log("Openwrt.FeatureRadio __init__ : START")
        DatabaseModule.__init__(self)
        ConnectionModules.__init__(self)
        self.db_obj = self.get_database_module_object()
        zi_logger.log(f"==== db_obj : {self.db_obj}")
        zi_logger.log("Openwrt.FeatureRadio __init__ : END")

    def load_wifi(self,
                  device):
        zi_logger.log(f"lib.openwrt.feature_radio.load_wifi({device})")
...
#            raise RuntimeError(f"Command execution failed : {command}")
    ...
    def set_channel(self,
                    ...
                    index,
                    channel):
        zi_logger.log(f"lib.openwrt.feature_radio.set_channel({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
        ...
        command = f"uci set wireless.{radio}.channel={channel}"
    ...
    def get_channel(self,
                    device,
                    index):
        zi_logger.log(f"lib.openwrt.feature_radio.get_channel({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def check_channel(self,
                      ...
                      index,
                      channel):
        zi_logger.log(f"lib.openwrt.feature_radio.check_channel({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def set_bandwidth(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.set_bandwidth({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def get_bandwidth(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_bandwidth({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci get wireless.{radio}.htmode"
        output, error = connection_obj.execute_command(command,
                                                       return_stderr=True)
        if error != '':
            raise RuntimeError(f"Command execution failed : {command}")
    ...
    def check_bandwidth(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.check_bandwidth({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def set_radio_state(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.set_radio_state({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
        ...
        if state:
            ...
            state = 1
        command = f"/sbin/uci set wireless.{radio}.disabled={state}"
    ...
    def get_radio_state(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_radio_state({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci get wireless.{radio}.disabled"
        output, error = connection_obj.execute_command(command,
                                                       return_stderr=True)
        if error != '':
            raise RuntimeError(f"Command execution failed : {command}")
    ...
    def set_radio_standard(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.set_radio_standard({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci set wireless.{radio}.hwmode={standard}"
        print(f"COMMAND : {command}")
        _, error = connection_obj.execute_command(command,
                                                  return_stderr=True)
        if error != '':
    ...
    def get_radio_standard(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_radio_standard({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci get wireless.{radio}.hwmode"
        output, error = connection_obj.execute_command(command,
                                                       return_stderr=True)
        if error != '':
            raise RuntimeError(f"Command execution failed : {command}")
    ...
    def set_txpower(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.set_txpower({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci set wireless.{radio}.txpower={txpower}"
        print(f"COMMAND : {command}")
        _, error = connection_obj.execute_command(command,
                                                  return_stderr=True)
        if error != '':
    ...
    def get_txpower(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_txpower({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def check_txpower(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.check_txpower({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"iw dev {ifname} info | grep txpower | awk '{{print$2}}'"
        ...
        if error != '':
            raise RuntimeError(f"Command execution failed : {command}")
        try:
            output_txpower = int(float(output.strip()))
        except ValueError:
            raise RuntimeError(f"Invalid txpower value: '{output}'")

        if output_txpower != txpower:
            raise RuntimeError(f"Given txpower {txpower} not matched with the expected txpower {output_txpower}")
    ...
    def set_noscan(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.set_noscan({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def get_noscan(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_noscan({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def set_regulatory_domain(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.set_regulatory_domain({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
#        try:
...
#            print(f"ERROR: {err}")
#            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"iw reg set {reg_domain}"
        print(f"COMMAND : {command}")
        _, error = connection_obj.execute_command(command,
                                                  return_stderr=True)
        if error != '':
    ...
    def set_country_regulatory_domain(self,
        ...
        To set the regulatory domain to a specific radio index
        """
        zi_logger.log(f"lib.openwrt.feature_radio.set_country_regulatory_domain({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci set wireless.{radio}.country={reg_domain}"
        print(f"COMMAND : {command}")
        _, error = connection_obj.execute_command(command,
                                                  return_stderr=True)
        if error != '':
    ...
    def get_regulatory_domain(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_regulatory_domain({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci get wireless.{radio}.country"
        output, error = connection_obj.execute_command(command,
                                                       return_stderr=True)
        if error != '':
            raise RuntimeError(f"Command execution failed : {command}")
    ...
    def __get_phy_index(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.__get_phy_index({device},{index})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def check_regulatory_domain(self,
        ...
        """
        zi_logger.log(f"lib.openwrt.feature_radio.check_regulatory_domain({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
    ...
    def get_physical_interface_index(self,
        ...
        Get the physical interface index for the given radio index
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_physical_interface_index({device},{index})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def get_supported_channels_list(self,
        ...
        zi_logger.log(f"lib.openwrt.feature_radio.get_supported_channels_list({device},{index})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def get_highest_supported_channel(self,
        ...
        zi_logger.log(f"lib.openwrt.feature_radio.get_highest_supported_channel({device},{index})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def get_supported_channels_count(self,
                                     ...
                                     index: str) -> int:  
        """Get the supported channels list for the given radio index"""
        zi_logger.log(f"lib.openwrt.feature_radio.get_supported_channels_count({device},{index})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
    ...
    def set_radio_type(self,
                       ...
                       index: str,
                       radio_type: str):
        """Set the wireless radio type for the specified radio index."""
        zi_logger.log(f"lib.openwrt.feature_radio.set_radio_type({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        try:
            ...
            print(f"ERROR: {err}")
            raise RuntimeError(f"Could not find out the Radio of the device : {device}")
        command = f"uci set wireless.{radio}.type='{radio_type}'"
        print(command)
        _, error = connection_obj.execute_command(command,
                                                  return_stderr=True)
        if error != '':
    ...
    def get_radio_type(self,
        ...
        """
        Get the wireless radio type for the specified radio index.
        """
        zi_logger.log(f"lib.openwrt.feature_radio.get_radio_type({device})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        ...
        command = f"uci get wireless.{radio}.type"
