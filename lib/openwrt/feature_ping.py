...
class FeaturePing(BaseFeaturePing,
    ...
    def ping_ipv4(self,
                  ...
                  count: int) -> int:
        """
        Ping from specified DUT device to destination ipv4 address.
        """
        zi_logger.log(f"lib.openwrt.feature_ping.ping_ipv4({device}, {dst_ip}, {count})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        command = f"ping -4 -c {count} {dst_ip}"
    ...
    def ping_ipv6(self,
                  ...
                  count: int) -> int:
        """
        Ping from specified DUT device to destination ipv6 address.
        """
        zi_logger.log(f"lib.openwrt.feature_ping.ping_ipv6({device}, {dst_ip}, {count})")
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
        connection_obj.switch_connection(device)
        command = f"ping -6 -c {count} {dst_ip}"
        output, error = connection_obj.execute_command(command,
                                                       return_stdout=True,
                                                       return_stderr=True)
        if error:
